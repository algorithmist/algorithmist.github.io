...
function CodeBlock(block)
  ...
  if fname then
    ...
    if block.attributes["id"] then
      imgObj.identifier = block.attributes["id"]
    end

    if block.attributes["animate"] then
      imgObj.attributes["onload"] = block.attributes["animate"]
    end

    -- Finally, put the image inside an empty paragraph. By returning the
    -- resulting paragraph object, the source code block gets replaced by
    -- the image:
    if block.attributes['script'] then
      return {
        pandoc.RawBlock('html', '<script src="' .. block.attributes['script'] .. '"></script>'),
        pandoc.Para {imgObj}
      }
    else
